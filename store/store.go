...
type GCRAStore interface {
	...
	// not exist. It also returns the current time at the Store. The time must
	...
	// SetIfNotExistsWithTTL sets the value of key only if it is not already set
	// in the store it returns whether a new value was set. If the store
	// supports expiring keys and a new value was set, the key will expire after
	// the provided ttl.
